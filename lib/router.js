...
Router.route('api-doc', { 
    name: 'apidoc',
    waitOn:function(){
        return Meteor.subscribe('allSlugsInDicts');
...
Router.route('/tipis/:_id', {
    ...
    waitOn: function () {
        return [
            ...
            Meteor.subscribe('relatedTipis', this.params._id),
            Meteor.subscribe('allDeputies')
        ];
    },
    data: function () {
        ...
        return {
            tipi: Tipis.findOne(id),
            related: _.toArray(_.filter(Tipis.find().fetch(), function(obj) { return obj._id !== id; }))
        }
    }
});
