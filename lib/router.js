...
Router.route('/t/:_id', {
    ...
    waitOn: function () {
        var oid = new Mongo.ObjectID(this.params._id);
        return Meteor.subscribe('singleTipiSummarized', oid);
    },
    data: function () {
        return {
            tipi: Refs.findOne()
        ...
        for (var k in cqry) {
            if( k == "fechadesde" && cqry[k] != "" ) {
                ...
                if (cqry['grupootro'] == 'Gobierno') {
                    newgrupootro = { 'autor.otro': cqry['grupootro'] }
                } else {
                    newgrupootro = { 'autor.grupo': cqry['grupootro'] }
                }
...
Router.route('/profile/:username', {
    name: 'profile',
    waitOn: function () {
        return Meteor.subscribe("userInfo", this.params.username);
    },
    data: function () {
...
Router.onBeforeAction(requireLogin, {only: ['postSubmit', 'blogPost']});
